...
PlayerManager::PlayerManager(CSDL_Setup* passed_csdl_setup, float *CameraX, float *CameraY)
{
	...
	player = new CSprite(csdl_setup->GetRenderer(),"data/new_mc_spritesheet.png",300,250,50,50,CameraX,CameraY, CCollisionRect(0,0,50,50));
}
...
bool PlayerManager::IsClose(CSprite* sprite, float rangeSpecified){
	float playerX = player->GetSpriteCameraX() - 20;
	float playerY = player->GetSpriteCameraY() - 40;
